...
const CATEGORIES = {
  appetizer:{key:"appetizer",emoji:"🐙",label:"Appetizers",subtitle:"Quick Movement",duration:5*60,color:"#C8A8E9",colorLight:"#EDE0F7",colorDark:"#9B6FCF",
    defaults:[{id:"a1",name:"Stretch & Shake",desc:"Streck dich wie ein Seestern 🌟"},{id:"a2",name:"Tanz-Break",desc:"30 Sek Lieblingslied tanzen 💃"},{id:"a3",name:"Kaltes Wasser",desc:"Hände unter kaltes Wasser & atmen 💧"},{id:"a4",name:"Hampelmann",desc:"20 Jumping Jacks! 🦘"},{id:"a5",name:"Fenster auf!",desc:"Frische Luft schnappen ☁️"}]},
...
/* ═══ SOFORTHILFE ═══ */
const SOFORTHILFE = [
  {id:"start",emoji:"🐢",animal:"Schildkröte",title:"Kann nicht anfangen",color:"#A8D8EA",colorDark:"#5BA3C0",colorLight:"#D6EEFB",science:"Exekutive Dysfunktion — Aufgabeninitiation bei ADHS beeinträchtigt (Barkley, 2012).",
    tips:[{id:"st1",icon:"🧩",title:"Mini-Schritt",text:"Nur die ersten 2 Minuten. Verhaltensmomentum senkt Widerstand (Nevin, 1996)."},{id:"st2",icon:"🎵",title:"Körper zuerst",text:"30 Sek Bewegung erhöht Dopamin (Ratey, 2008)."},{id:"st3",icon:"📢",title:"Laut aussprechen",text:"Kompensiert schwächeres inneres Sprechen (Barkley, 1997)."},{id:"st4",icon:"👯",title:"Body Doubling",text:"Neben jemandem arbeiten erhöht Aufgabenbindung (Zajonc, 1965)."}]},
  {id:"overwhelm",emoji:"🐻",animal:"Bär",title:"Alles zu viel",color:"#F4A0B5",colorDark:"#D06B83",colorLight:"#FCE0E8",science:"Kognitive Überlast — geringere Arbeitsgedächtnis-Kapazität (Martinussen, 2005).",
    tips:[{id:"ow1",icon:"📝",title:"Brain Dump",text:"Alles aufschreiben befreit Arbeitsgedächtnis (Sweller, 1988)."},{id:"ow2",icon:"3️⃣",title:"Nur 3 Dinge",text:"Reduziert Entscheidungsmüdigkeit (Baumeister, 1998)."},{id:"ow3",icon:"🫁",title:"Box Breathing",text:"4-4-4-4. Senkt Cortisol (Ma et al., 2017)."},{id:"ow4",icon:"🧸",title:"Komfort-Anker",text:"Vertraute Reize aktivieren Sicherheitssystem (Porges, 2011)."}]},
  {id:"focus",emoji:"🦋",animal:"Schmetterling",title:"Kann mich nicht konzentrieren",color:"#C8A8E9",colorDark:"#9B6FCF",colorLight:"#EDE0F7",science:"Brown Noise verbessert Leistung bei ADHS (Söderlund, 2007).",
    tips:[{id:"fo1",icon:"🎧",title:"Brown Noise",text:"Erhöht Dopamin im präfrontalen Kortex."},{id:"fo2",icon:"📱",title:"Handy weg",text:"Sichtbare Nähe reduziert kognitive Kapazität (Ward, 2017)."},{id:"fo3",icon:"⏱️",title:"5-Min-Sprint",text:"Zeigarnik-Effekt motiviert zum Weitermachen."},{id:"fo4",icon:"✍️",title:"Ablenkungszettel",text:"Gedanken notieren reduziert Intrusions (Wegner, 1994)."}]},
  {id:"restless",emoji:"🐒",animal:"Äffchen",title:"Unruhig & zappelig",color:"#FFD6A0",colorDark:"#CC9544",colorLight:"#FFF0D6",science:"Fidgeting verbessert kognitive Leistung bei ADHS (Hartanto, 2016).",
    tips:[{id:"re1",icon:"🏃",title:"Bewegungs-Snack",text:"10 Kniebeugen erhöhen Dopamin (Ratey, 2008)."},{id:"re2",icon:"🧊",title:"Sensorik-Reset",text:"Propriozeptiver Input beruhigt (Ayres, 1972)."},{id:"re3",icon:"🪑",title:"Position wechseln",text:"Aktiviert Orientierungsnetzwerk (Posner, 1990)."},{id:"re4",icon:"🎵",title:"Rhythmisches Klopfen",text:"Bilaterale Stimulation beruhigt (Shapiro, 2001)."}]},
  {id:"decisions",emoji:"🦔",animal:"Igel",title:"Kann mich nicht entscheiden",color:"#B8D8A8",colorDark:"#6FA05A",colorLight:"#E0F0D6",science:"Analyse-Paralyse bei ADHS (Damasio, 1994).",
    tips:[{id:"de1",icon:"🪙",title:"Münze werfen",text:"Dein Gefühl zeigt die wahre Präferenz."},{id:"de2",icon:"⏰",title:"2-Min-Timer",text:"Satisficing statt Maximizing (Schwartz, 2004)."},{id:"de3",icon:"🤏",title:"Kleinste Version",text:"Geringstem Aufwand wählen."},{id:"de4",icon:"🗣️",title:"Laut denken",text:"Kompensiert Arbeitsgedächtnis-Defizite."}]},
  {id:"emotions",emoji:"🐨",animal:"Koala",title:"Gefühle überfluten mich",color:"#A8C8E8",colorDark:"#5A8AB8",colorLight:"#D6E8F8",science:"Affect Labeling reduziert Amygdala-Aktivität (Lieberman, 2007).",
    tips:[{id:"em1",icon:"🏷️",title:"Gefühl benennen",text:"Reduziert Amygdala-Reaktivität um ~50%."},{id:"em2",icon:"🌡️",title:"Dive Reflex",text:"Kaltes Wasser senkt Herzfrequenz (Linehan, 1993)."},{id:"em3",icon:"📍",title:"5-4-3-2-1 Grounding",text:"5 sehen, 4 hören, 3 fühlen, 2 riechen, 1 schmecken."},{id:"em4",icon:"⏳",title:"90-Sekunden-Regel",text:"Emotion dauert ~90 Sek (Bolte Taylor, 2006)."}]},
  {id:"timeblind",emoji:"🐌",animal:"Schnecke",title:"Zeitgefühl verloren",color:"#E8C8A8",colorDark:"#B8945A",colorLight:"#F5E8D6",science:"Zeitblindheit bei ADHS (Barkley, 1997).",
    tips:[{id:"tb1",icon:"⏰",title:"Visuelle Timer",text:"Zeit sichtbar machen."},{id:"tb2",icon:"🔔",title:"3er-Alarm-Kette",text:"3 Alarme statt einem."},{id:"tb3",icon:"⏪",title:"Rückwärts planen",text:"Vom Termin zurückrechnen."},{id:"tb4",icon:"🎯",title:"Zeit schätzen",text:"Raten, dann stoppen — wird besser."}]},
  {id:"forget",emoji:"🐘",animal:"Elefant",title:"Vergesse ständig Dinge",color:"#D8B8D8",colorDark:"#A07AA0",colorLight:"#F0E0F0",science:"Arbeitsgedächtnis-Defizit (Martinussen, 2005).",
    tips:[{id:"fg1",icon:"📍",title:"Launch Pad",text:"Fester Platz an der Tür (Barkley, 2012)."},{id:"fg2",icon:"🗒️",title:"Sofort notieren",text:"In 3 Sek ist der Gedanke weg."},{id:"fg3",icon:"🔁",title:"Habit Stacking",text:"An bestehende Gewohnheit ketten (Gollwitzer, 1999)."},{id:"fg4",icon:"📸",title:"Foto-Gedächtnis",text:"Fotografiere was du vergessen könntest."}]},
];
...
function buildNotifications(habits,settings){
  ...
  if(settings?.dailyTip){const sh=SOFORTHILFE[Math.floor(Math.random()*SOFORTHILFE.length)];const t=sh.tips[Math.floor(Math.random()*sh.tips.length)];notifs.push({type:"tip",msg:`${sh.emoji} ${t.title}: ${t.text}`,color:"#C8A8E9",helpId:sh.id});}
  if(settings?.dailyMenu){const cats=Object.values(CATEGORIES);const c=cats[Math.floor(Math.random()*cats.length)];const it=c.defaults[Math.floor(Math.random()*c.defaults.length)];notifs.push({type:"menu",msg:`${c.emoji} Menü-Tipp: ${it.name} — ${it.desc}`,color:"#B8E8D0"});}
  return notifs;
}
...
export default function DopaminMenu(){
  ...
  // persist
  useEffect(()=>{sv({items,stars,streak,lastDate,doneToday,habits,customTips,catOrder,notifSettings});},[items,stars,streak,lastDate,doneToday,habits,customTips,catOrder,notifSettings]);
  ...
  const addCT=pid=>{if(!tTitle.trim())return;setCustomTips(p=>({...p,[pid]:[...(p[pid]||[]),{id:"ct_"+Date.now(),icon:tIcon,title:tTitle.trim(),text:tText.trim()||"",custom:true}]}));setTIcon("💡");setTTitle("");setTText("");setAddTipFor(null);};
  ...
  const getQT=cid=>{const cat=BAT_CATS.find(c=>c.id===cid);const ch=habitsFor(cid);const ov=ch.filter(h=>{const iv=INTERVALS.find(i=>i.id===h.interval)||INTERVALS[2];return isOver(h.lastDone,iv.days);});const hp=SOFORTHILFE.find(p=>p.id===cat?.helpId);const tips=[];if(ov.length){const e=ov.reduce((a,b)=>(a.weight||1)<(b.weight||1)?a:b);const tw=ch.reduce((s,h)=>s+(h.weight||1),0);tips.push({icon:"⚡",title:"Schnellster Boost: "+e.name,text:`+${Math.round(((e.weight||1)/tw)*100)}%`,action:()=>checkIn(e.id)});}if(hp?.tips.length){const t=hp.tips[Math.floor(Math.random()*hp.tips.length)];tips.push({icon:hp.emoji,title:t.title,text:t.text,link:cat?.helpId});}tips.push({icon:"🧸",title:"Nur 1 Minute",text:"Timer auf 60 Sek. Tu das Einfachste."});return tips.slice(0,3);};
  ...
  return(
    <div style={S.wrap}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Fredoka:wght@400;500;600;700&display=swap');*{box-sizing:border-box;margin:0;padding:0}@keyframes wiggle{0%,100%{transform:rotate(-5deg)}50%{transform:rotate(5deg)}}@keyframes nod{0%,100%{transform:translateY(0)}50%{transform:translateY(-6px)}}@keyframes bounce{0%,100%{transform:translateY(0) scaleY(1)}50%{transform:translateY(-8px) scaleY(.95)}}@keyframes pop{0%{transform:scale(.3);opacity:0}60%{transform:scale(1.15);opacity:1}100%{transform:scale(1)}}@keyframes confetti{0%{transform:translateY(0) rotate(0);opacity:1}100%{transform:translateY(-120px) rotate(720deg);opacity:0}}@keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-10px)}}@keyframes fadeIn{0%{opacity:0;transform:translateY(12px)}100%{opacity:1;transform:translateY(0)}}@keyframes pulse{0%,100%{opacity:1}50%{opacity:.6}}@keyframes checkPop{0%{transform:scale(1)}40%{transform:scale(1.25)}100%{transform:scale(1)}}@media(min-width:768px){.tablet-grid{grid-template-columns:repeat(auto-fill,minmax(180px,1fr))!important}.bat-grid-t{grid-template-columns:repeat(auto-fill,minmax(120px,1fr))!important}}`}</style>
      ...
      {showNotifs&&<div style={S.notifPanel}>
        ...
        {showNotifSettings&&<div style={S.notifSettingsBox}>
          ...
          <label style={S.notifToggle}><input type="checkbox" checked={notifSettings.dailyMenu} onChange={e=>setNotifSettings(p=>({...p,dailyMenu:e.target.checked}))}/><span>🍽️ Täglicher Menü-Vorschlag</span></label>
      ...
      {tab==="hilfe"&&!timerOn&&<div style={{padding:"4px 16px"}}><div style={S.hI}><Caterpillar size={40} animate/><p style={{fontSize:12,color:"#5B4A6A",fontWeight:500,lineHeight:1.4}}>Was macht dir gerade zu schaffen? 💛</p></div>
        {!openProb?(<div className="tablet-grid" style={S.pG}>{SOFORTHILFE.map((p,i)=>(<button key={p.id} style={{...S.pC,background:p.colorLight,borderColor:p.color,animation:`fadeIn .3s ease-out ${i*.04}s both`}} onClick={()=>setOpenProb(p.id)}><MiniAnimal type={p.emoji} size={44}/><span style={{fontSize:11.5,fontWeight:600,textAlign:"center",lineHeight:1.25,color:p.colorDark}}>{p.title}</span><span style={{fontSize:9.5,fontWeight:500,color:"#9B8AAE"}}>{p.animal}</span></button>))}</div>)
        :(()=>{const pr=SOFORTHILFE.find(p=>p.id===openProb);const tips=allTips(openProb);return(<div style={{animation:"fadeIn .3s ease-out"}}><button style={S.bk} onClick={()=>{setOpenProb(null);setAddTipFor(null);}}>← Zurück</button>
          <div style={{display:"flex",alignItems:"center",gap:9,padding:"10px 12px",borderRadius:16,marginBottom:8,background:pr.colorLight}}><MiniAnimal type={pr.emoji} size={50}/><div style={{flex:1}}><h2 style={{fontSize:17,fontWeight:700,color:pr.colorDark}}>{pr.title}</h2><p style={{fontSize:11,fontWeight:500,opacity:.8,color:pr.colorDark}}>{pr.animal} hat Tipps 💛</p></div><button style={{background:"#fff",border:`2px solid ${pr.color}`,borderRadius:9,width:32,height:32,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,cursor:"pointer"}} onClick={()=>setShowSci(showSci===pr.id?null:pr.id)}>🔬</button></div>
          {showSci===pr.id&&<div style={{borderRadius:12,border:`2px solid ${pr.color}`,padding:"8px 12px",marginBottom:8,background:pr.colorLight}}><p style={{fontSize:11.5,lineHeight:1.5,fontWeight:500,color:pr.colorDark}}>📚 {pr.science}</p></div>}
          <div style={{display:"flex",flexDirection:"column",gap:8}}>{tips.map((t,i)=>(<div key={t.id} style={{background:"#fff",borderRadius:14,padding:11,border:`2px solid ${pr.color}`,display:"flex",gap:9,alignItems:"flex-start",animation:`fadeIn .3s ease-out ${i*.05}s both`}}><div style={{width:36,height:36,borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",fontSize:17,flexShrink:0,background:pr.colorLight}}>{t.icon}</div><div style={{flex:1}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><h3 style={{fontSize:13,fontWeight:600,color:pr.colorDark,marginBottom:2}}>{t.title}</h3>{t.custom&&<button style={S.rm} onClick={()=>rmCT(pr.id,t.id)}>×</button>}</div><p style={{fontSize:11.5,color:"#5B4A6A",lineHeight:1.45}}>{t.text}</p></div></div>))}</div>
      ...
      {tab==="batterie"&&!timerOn&&<div style={{padding:"4px 16px"}}>
        ...
        {!openCat?(<>
          ...
          return(<div style={{animation:"fadeIn .3s ease-out"}}><button style={S.bk} onClick={()=>{setOpenCat(null);setShowHF(false);}}>← Alle Batterien</button>
            <div style={{display:"flex",alignItems:"center",gap:14,padding:"14px 16px",borderRadius:20,marginBottom:12,background:bc.colorLight}}><BatterySVG pct={ch.length?pct:100} size={90}/><div style={{flex:1}}><h2 style={{fontSize:20,fontWeight:700,color:bc.colorDark}}>{bc.emoji} {bc.label}</h2><p style={{fontSize:12,color:bc.colorDark,fontWeight:500,marginTop:2}}>{getBatLabel(ch.length?pct:100)}</p><p style={{fontSize:11,color:"#9B8AAE",marginTop:4}}>{ch.length} Habit{ch.length!==1?"s":""}</p></div></div>
            {pct<=40&&ch.length>0&&<div style={S.qT}><h3 style={{fontSize:14,fontWeight:700,color:"#B83A3A",marginBottom:8}}>⚡ Schnell aufladen:</h3>{qt.map((q,i)=>(<div key={i} style={{background:"#fff",borderRadius:14,padding:11,display:"flex",gap:9,alignItems:"flex-start",marginBottom:6,animation:`fadeIn .3s ease-out ${i*.08}s both`}}><div style={{width:34,height:34,borderRadius:10,background:"#FDE8E6",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>{q.icon}</div><div style={{flex:1}}><h4 style={{fontSize:13,fontWeight:600,color:"#5B4A6A"}}>{q.title}</h4><p style={{fontSize:11,color:"#6B5F7F",lineHeight:1.4,marginTop:2}}>{q.text}</p></div>{q.action&&<button style={{background:"#E86A5A",color:"#fff",border:"none",borderRadius:10,padding:"6px 12px",fontSize:11,fontWeight:600,fontFamily:"Fredoka,sans-serif",cursor:"pointer",alignSelf:"center"}} onClick={q.action}>Jetzt!</button>}{q.link&&<button style={{background:"none",border:"none",fontSize:11,fontWeight:600,color:"#5BA3C0",fontFamily:"Fredoka,sans-serif",cursor:"pointer",alignSelf:"center"}} onClick={()=>goToHelp(q.link)}>Hilfe →</button>}</div>))}</div>}
            {ch.length>0&&<div style={{display:"flex",flexDirection:"column",gap:9,marginBottom:12}}>{ch.map((h,idx)=>{const iv=INTERVALS.find(i=>i.id===h.interval)||INTERVALS[2];const ov=isOver(h.lastDone,iv.days);const np=Math.round(((h.weight||1)/totalW)*100);const rm=REMIND_OPTS.find(r=>r.id===h.remind);return(<div key={h.id} style={{background:"#fff",borderRadius:16,padding:13,border:`2.5px solid ${ov?"#E86A5A":"#5EC269"}`,animation:`fadeIn .3s ease-out ${idx*.05}s both`}}><div style={{display:"flex",alignItems:"center",gap:9}}><div style={{width:12,height:12,borderRadius:6,background:ov?"#E86A5A":"#5EC269",flexShrink:0}}/><div style={{flex:1,minWidth:0}}><h3 style={{fontSize:14,fontWeight:700,color:"#4A3D5C"}}>{h.name}</h3><p style={{fontSize:10.5,color:"#9B8AAE",fontWeight:500,marginTop:1}}>{iv.label} · {np}% · {timeAgo(h.lastDone)}{rm&&rm.id!=="none"?` · 🔔 ${rm.label}`:""}</p></div><button style={{background:"none",border:"none",fontSize:13,cursor:"pointer",opacity:.4}} onClick={()=>rmH(h.id)}>🗑️</button></div><button style={{border:"none",borderRadius:12,padding:"9px 0",fontSize:12.5,fontWeight:600,fontFamily:"Fredoka,sans-serif",cursor:"pointer",color:"#fff",marginTop:8,width:"100%",background:ov?"#E86A5A":"#5EC269",animation:checkAnim===h.id?"checkPop .4s ease-out":"none"}} onClick={()=>checkIn(h.id)}>{ov?`Aufladen +${np}%`:"Erledigt ✓"}</button></div>);})}</div>}
            ...
            {showHF?(<div style={{background:"#fff",borderRadius:18,padding:16,boxShadow:"0 4px 16px rgba(180,160,200,.12)",animation:"fadeIn .3s ease-out"}}><h3 style={{fontSize:16,fontWeight:700,color:"#5B4A6A",marginBottom:10}}>Neuer Habit für {bc.emoji} {bc.label}</h3>
              ...
              <label style={S.fL}>🔔 Erinnerung</label><div style={{display:"flex",flexWrap:"wrap",gap:5}}>{REMIND_OPTS.map(ro=>(<button key={ro.id} style={{border:"none",borderRadius:12,padding:"7px 12px",fontSize:12,fontWeight:600,fontFamily:"Fredoka,sans-serif",cursor:"pointer",background:hRemind===ro.id?"#FFB5A7":"#FFE0DA",color:hRemind===ro.id?"#fff":"#E8876F"}} onClick={()=>setHRemind(ro.id)}>{ro.label}</button>))}</div>
